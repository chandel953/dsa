...
ll lcm(ll a,ll b) { return a/gcd(a,b)*b; }
 ...
 int n ,m ;
    ...
    int egg(int i ,int j){
        ...
        int mini = INT_MAX ;
        for(int k = i ;k<=j-1 ;k++){
            int temp = max(egg(i-1 ,k-1) ,egg(k,j-k)) ;
            mini = min(mini ,temp) ;
        }
        return mini ;
    }
    ...
    int knap(vector<int>&wt ,vector<int>&v ,int i ,int w ,vector<vector<int>>&dp){
        ...
        int pick= 0 ;
        if(w>=wt[i]){
            pick = knap(wt ,v ,i ,w- wt[i],dp) + v[i];
        }
        int npick = knap(wt ,v,i-1,w,dp) ;

        return dp[i][w] = max(pick ,npick) ;
    }
    int coins(vector<int>&coin ,int amount ,int i,vector<vector<int>>&dp){
        if(i==0){
        ...
        int npick = coins(coin ,amount , i-1,dp) ;
        int pick = 1e8 ;
        if(amount>=coin[i]){
            pick = coins(coin ,amount- coin[i] ,i,dp) +1;
        }
